 ...
 public function checkin()
    {
        ...
        $ledger->perticulation = 'Bonus diario de check-in';
   ...
   public function checkinn()
    ...
    public function appreview_submit(Request $request)
    {
        $validate = Validator::make($request->all(), [
        ...
        $proof = new SocialProof();
        $proof->user_id = Auth::id();
        $proof->message = $request->message;
        $proof->payout_amount = rand(1, 3);
        $proof->status = 'pending';

        $path = uploadImage(false, $request, 'photo', 'upload/social-proof/', null, null, $proof->photo);
